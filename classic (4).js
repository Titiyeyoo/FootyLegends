...
(function(){
  ...
  function renderSearch(){
    ...
    if($("cl-search-label")) $("cl-search-label").innerHTML = "Add "+slot.pos+(slot.player?' <span class="cl-clear" onclick="clClear()">✕ remove '+clLast(slot.player.n)+'</span>':'');
  }
  ...
  window.clViewXI = function(){
    if(CL.slots.filter(s=>s.player).length<11) return;
    CL.target=null; CL.sugg=[];
