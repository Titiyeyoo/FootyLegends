...
function openDraftEmpty(){
  // ===== TYMCZASOWA DIAGNOSTYKA (usunac po znalezieniu przyczyny) =====
  try{
    var _raw = localStorage.getItem("fl_xad_draft");
    if(!_raw){ alert("DIAG: brak zapisu w localStorage"); }
    else { var _b=JSON.parse(_raw); alert("DIAG: zapis JEST | sv:"+_b.sv+" | pool:"+(_b.pool?_b.pool.length:"?")+" | picked:"+((_b.slots||[]).filter(function(x){return x;}).length)+" | wiek(min):"+Math.round((Date.now()-(_b.ts||0))/60000)); }
  }catch(e){ alert("DIAG blad: "+e); }
  // ===== KONIEC DIAGNOSTYKI =====
...
function flRestoreDraft(b){
  try{
    ...
    POOL = b.pool || [];
    buildSlots();
    for(var i=0;i<SLOTS.length && i<b.slots.length;i++){ if(b.slots[i]) SLOTS[i].player = b.slots[i]; }
...
function flFootballLife(games){
  if(!games || games<1) return "YOUR STORY STARTS HERE";
  var mins=Math.round(flGetPlayMs()/60000), t;
  if(mins<60) t=mins+"M"; else { var h=Math.floor(mins/60), m=mins%60; t=h+"H"+(m?(" "+m+"M"):""); }
...
function placePlayer(p){
  ...
  slot.player = p; flash("");
  if(picked()===1 && !FL_BUILD_START){ FL_BUILD_START = Date.now(); try{ flTrack("first_pick",{slot:p.p, era:p.e}); }catch(e){} } // pierwszy pick po rollu (timer + lejek)
...
function buildRadarSVG(life, form){
  ...
  svg+='<polygon points="'+flPoly(disp(life),cx,cy,R)+'" fill="rgba(233,196,106,0.26)" stroke="#e9c46a" stroke-width="2.5"/>';
...
function flBuildDnaCard(cb){
  ...
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
  ctx.fillStyle="#aca792"; ctx.font="500 30px Arial, sans-serif";
