...
function openLegacy(){
  ...
  const _foundCard = s => '<div class="lg-card found r-'+s.r.toLowerCase()+'" onclick="xadShowSquad(\''+s.k+'\')"><div class="lg-emoji">'+(s.k==="class92"?XAD_DEVIL_SVG:s.e)+'</div><div class="lg-name">'+s.n+'</div><div class="lg-rar">'+s.r+'</div></div>';
  ...
  let games = 0;
  try { const _pr = JSON.parse(localStorage.getItem('fl_profile') || 'null'); if(_pr && _pr.games) games = _pr.games; } catch(e){}
  const body = document.getElementById('legacy-body');
  if(!body) return;
  body.innerHTML =
    '<div class="lg-sec"><span class="lg-ic">🏛️</span> YOUR JOURNEY</div>'+
    '<div class="lg-journey">'+
      '<div class="lg-jstat"><span>LEGENDS</span><b>'+qCount+' / '+qTotal+'</b></div>'+
      '<div class="lg-jstat"><span>NARRATIVES</span><b>'+found+' / '+total+'</b></div>'+
      '<div class="lg-jstat"><span>OBSERVATIONS</span><b>—</b></div>'+
      '<div class="lg-jstat"><span>GAMES</span><b>'+games+'</b></div>'+
    '</div>'+
    '<div class="lg-divider"></div>'+
    '<div class="lg-sec xad"><span class="lg-ic">🃏</span> NARRATIVES</div>'+
    '<div class="lg-count">DISCOVERED <b>'+found+' / '+total+'</b></div>'+
    ...
    '<div class="lg-hint" style="padding:6px 0 2px;line-height:1.7">the game is starting to notice how you play…<br>your first observations will appear here.</div>'+
    '<div class="lg-divider"></div>'+
    '<div class="lg-sec quiz"><span class="lg-ic">📋</span> QUICK QUIZ</div>'+
    '<div class="lg-count">LEGENDS COLLECTED <b>'+qCount+' / '+qTotal+'</b></div>'+
