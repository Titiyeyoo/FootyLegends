...
(function(){
  ...
  window.clOpenNew = function(){
    ...
    if(sv.length){
      ...
      sv.slice(0,CL_SAVE_MAX).forEach(function(x){
        ...
        h+='<button class="cl-new-i cl-new-s" onclick="clOpenSaved(\''+x.id+'\')">💾 '+clEsc(x.name||"MY ENGLAND XI")+' <span class="cl-new-dim">'+n+'/11</span></button>';
      });
  ...
  window.clSaveXI = function(){
    ...
    var label=name || "MY ENGLAND XI";
    var doSave=function(id){
      var rec={ id:id||("xi_"+Date.now()), name:label, formation:CL.formation,
      ...
      if(clStoreSaved(list)){ CL_SAVED_ID=rec.id; clToast("XI SAVED"); }
    };
    ...
    var rows = list.length ? list.map(function(x){
      ...
      return '<div class="cl-sv-row">'+
        '<button class="cl-sv-open" onclick="clOpenSaved(\''+x.id+'\')">'+
          '<span class="cl-sv-n">'+clEsc(x.name||"MY ENGLAND XI")+'</span>'+
          '<span class="cl-sv-m">'+x.formation+' · '+n+'/11 · '+(d.getDate())+'/'+(d.getMonth()+1)+'</span>'+
        '</button>'+
  ...
  function clToast(txt){
    var t=document.createElement('div'); t.className='cl-toast'; t.textContent=txt;
