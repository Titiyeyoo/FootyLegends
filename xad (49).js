...
function drawSquad(){
  ...
  if(!dice){ POOL=buildPool(); REROLL_USED=false; DEALT=true; POS_FILTER=null; render(); flash(""); const sx=el("s-xad"); if(sx) sx.scrollTop=0; return; }
  ROLLING = true;
...
function render(animate, justJumped){
  ...
  SLOTS.forEach((s,i) => {
    ...
    if(s.player){
      ...
      node.onclick = DEALT ? (() => filterBySlot(s.p)) : (() => { try{ window.xadDraw(); }catch(e){} });
    }
...
function saveXadSquad(key, score, name){
  ...
  try{
    ...
    all[key] = {
      key:key, name:name||key, formation:FORMATION, score:score, ts:Date.now(),
...
function lockXI(){
  ...
  try{ if(det.main) saveXadSquad(det.main, score, NARRATIVES[det.main].name); }catch(e){}
