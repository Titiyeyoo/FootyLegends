...
function confirmChangeName(){
  ...
  // Puste imie JEST dozwolone — na tablicy wynikow pokaze sie jako GUEST.
  setStoredName(v);
  G.playerName = v || 'GUEST';
  // Update any visible name displays
  refreshPlayingAs();
  const rName = document.getElementById('r-name');
  if(rName) rName.textContent = v;
  closeChangeNameModal();
  toast(v ? '✓ NAME UPDATED' : '✓ NAME CLEARED','');
}
...
function handleSubmit(){
  ...
  if(G.pendingSurname){
    ...
    if(!resolved && norm(v).length >= 3){
      ...
      if(fuzzyMatches.length === 1){
        ...
        // No extra toast — handleCorrect() shows centred "✓ NAME" popup over input.
        // Toast here was causing double name flash (toast + popup with same name).
  ...
  if(v.length >= 3){
    ...
    if(almostMatch){
      // Auto-accept typo. Player gets full feedback via showQuizPopup() inside
      // handleCorrect() (centred "✓ NAME" popup over input). No extra toast needed
      // — was causing double name flash (toast + popup showing same name).
...
function endGame(){
  ...
  try{flTrack("game_complete",{mode:"quiz", score:score});}catch(e){}
...
function playAgain(){ try{flTrack("play_again",{mode:"quiz"});}catch(e){}
  startEra(G.era || 'ALL');
}
...
// SHARE
// ══════════════════════════════════════════════════
function buildShareText(gameOver, includeUrl){ try{flTrack("share",{mode:"quiz"});}catch(e){}
...
function shareFB(gameOver){
  try{flTrack("share",{mode:"quiz"});}catch(e){}
...
function setStoredName(name){
  try {
    if(name) localStorage.setItem('fl_player_name', name);
    else     localStorage.removeItem('fl_player_name');   // pozwalamy skasowac
  } catch(e){}
}
...
function renderHOF(era){
  ...
  el.innerHTML = header + hof.map((e,i)=>`
    <div class="hof-row">
      ...
      <div class="hof-name">${escapeHtml(e.name)}
        <div class="hof-date">${escapeHtml(e.date)}</div>
      </div>
...
function updateCollectionStats(){
  // XI After Dark — Legacy (główny tryb): N / 17
  let xadFound = 0;
  try{ xadFound = JSON.parse(localStorage.getItem("fl_xad_stories")||"[]").length; }catch(e){}
  const xadTotal = getXadStoryList().length || 20;
  const xEl = document.getElementById('opt-xad-stories');
  if(xEl) xEl.textContent = xadFound + " / " + xadTotal;
  const xBar = document.getElementById('opt-xad-bar');
  if(xBar) xBar.style.width = Math.min(100, Math.round((xadFound / xadTotal) * 100)) + "%";
...
function renderPubLives(){
  ...
  row.innerHTML = pubSession.teams.map((t,i)=>{
    ...
    return '<div style="display:flex;justify-content:space-between;align-items:center;font-size:10px;letter-spacing:2px;padding:6px 8px;' + bgStyle + style + '">' +
      '<span style="color:' + color + ';font-weight:' + weight + '">' + (isActive ? "▶ " : "  ") + escapeHtml(t.name) + '</span>' +
...
function pubSubmit(){
  ...
  // ── STEP 3: fuzzy match — auto-accept typos ("beckam" → Beckham) ──
  // Mirror of findAlmost() path in quiz handleSubmit. Uses fuzzyMatchAlias
  // primitive (core.js) — same algorithm, pool-agnostic. Sylwia 17.05.
  const v = norm(raw);
  if(v.length >= 3){
    const almostMatch = fuzzyMatchAlias(pubSession.pool, v, pubSession.usedPlayers);
    if(almostMatch){
      pubAcceptAnswer(almostMatch);
      return;
    }
  }
    ...
    board.innerHTML = sorted.map((t,i)=>{
      ...
      return '<div>' +
        '<div ' + (hasGuesses ? 'onclick="togglePubGuessList(' + i + ')"' : '') +
          ...
          '<span style="color:' + color + ';font-weight:' + (isWinner ? 'bold' : 'normal') + ';flex:1">' + medal + ' ' + escapeHtml(t.name) + chevron + '</span>' +
...
function registerServiceWorker(){
  ...
  navigator.serviceWorker.register('service-worker.js')
    .then(reg => {
      ...
      // Przy powrocie do apki (visibilitychange) sprawdz, czy jest nowa wersja SW —
      // dlugo otwarta sesja wykryje deploy bez pelnego zamykania apki.
...
function resetHOF(){
  if(!confirm("Reset everything? Scores, discovered legends, stories, saved squads, streak and unlocks will all be wiped. This can't be undone.")) return;
  // pełny reset postępu — czyścimy wszystko poza tożsamością gracza, dźwiękiem i wersją aplikacji
  const KEEP = ['fl_player_id','fl_player_name','fl_sound','fl_app_version'];
  const toClear = [
    'fl_hof_ALL','fl_hof_C','fl_hof_G','fl_hof_M','fl_xi_hof',
    'fl_discovered','fl_discovered_order',
    'fl_daily_streak','fl_last_play','fl_last_era',
    'fl_blind_first_seen','fl_blind_unlock_pending','fl_blind_unlocked_seen',
    'fl_xi_first_seen','fl_xi_unlock_pending','fl_xi_unlocked',
    'fl_xad_stories','fl_xad_squads',
    'fl_has_played'
  ];
  toClear.forEach(k => { if(!KEEP.includes(k)) localStorage.removeItem(k); });
  // sprzątanie ewentualnych innych kluczy postępu (fl_hof_*) bez ruszania KEEP
  for(let i = localStorage.length - 1; i >= 0; i--){
    const k = localStorage.key(i);
    if(k && k.indexOf('fl_hof_') === 0 && !KEEP.includes(k)) localStorage.removeItem(k);
  }
  toast("✓ EVERYTHING RESET","");
  // odśwież ekran ustawień (streak, stories, licznik legend, tier) jeśli widoczny
  if(typeof updateSoundBtn === "function") updateSoundBtn();
  else if(typeof updateCollectionStats === "function") updateCollectionStats();
  const hofEl = document.getElementById('s-hof');
  if(hofEl && hofEl.classList.contains('on') && typeof renderHOF === "function") renderHOF('ALL');
}
...
function updateDot(el,s,i){
  ...
  // Show only surname for readability. Full name in `title` attribute for desktop
  // hover tooltip (Sylwia 17.05) — solves overflow truncation for long surnames
  // like ALEXANDER-ARNOLD, RICHARDSON, GIBBS-WHITE. Mobile users see truncated
  // surname only (browser hover doesn't fire on touch).
  if(filled){
    const parts = s.name.split(" ");
    n.textContent = parts[parts.length-1]; // last word = surname
    el.title = s.dbName || s.name; // full name on hover
  } else {
    n.textContent = "";
...
function renderLockedPitch(){
  ...
  slots.forEach((s,i)=>{
    ...
    // Note: title attribute would not work here because parent #wrap-locked has
    // pointer-events:none. If we ever want hover tooltip in locked XI, override
    // pointer-events:auto on .pdot-locked. For now, locked is read-only display.
    el.innerHTML =
...
function shareLockedResult(){
  if(!lastChem) return;
  try{flTrack("share",{mode:"classic"});}catch(e){}
  const c = lastChem;
