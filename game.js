...
function openXI(){
  // Must be unlocked via blind draft completion
  if(localStorage.getItem('fl_xi_unlocked') !== '1'){
    return; // button should be disabled/hidden, but guard anyway
  }
...
function updateSoundBtn(){
  ...
  if(btn){
    ...
    btn.style.borderColor = on ? "var(--green)" : "#555";
    btn.style.color       = on ? "var(--green)" : "#777";
  }
...
function updateCollectionStats(){
  ...
  if(bEl){
    ...
    if(badge){
      ...
      const next = BADGES[0];
      bEl.textContent = "NONE (NEXT: " + (next.at - count) + ")";
      bEl.style.color = "#888";
    }
  }
}
...
function drawBlindDraft(){
  ...
  if(allDiscovered.length < 30){
    alert("Need at least 30 known players in your collection. You have " + allDiscovered.length + ".");
    return;
  }
  ...
  const drawn = shuffled.slice(0, 30);
