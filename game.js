...
function xadShowSquad(key){
  ...
  if(!sq){
    ...
    var d=new Date(sq.ts||Date.now());
    var M=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
    var dstr=d.getDate()+' '+M[d.getMonth()]+' '+d.getFullYear();
    inner='<div class="xsv-card" onclick="event.stopPropagation()">'+
            '<div class="xsv-name">'+emoji+' '+_xsvEsc(name)+'</div>'+
            '<div class="xsv-meta">'+sq.score+' PTS · '+_xsvEsc(sq.formation)+'</div>'+
            '<div class="xad-pitch xad-pitch-reveal"><div class="xad-penalty"></div><div class="xad-sixyard"></div>'+dots+'</div>'+
            '<div class="xsv-date">'+dstr+'</div>'+
          '</div>';
  }
  ov.innerHTML=inner;
...
function openLegacy(){
  ...
  body.innerHTML =
    ...
    (found ? '<div class="lg-grid">'+discGrid+'</div>' : '')+
