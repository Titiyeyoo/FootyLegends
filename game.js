...
function openLegacy(){
  ...
  let grid = '';
  XAD_STORIES.forEach(s => {
    if(disc.has(s.k)){
      grid += '<div class="lg-card found r-'+s.r.toLowerCase()+'"><div class="lg-emoji">'+(s.k==="class92"?XAD_DEVIL_SVG:s.e)+'</div><div class="lg-name">'+s.n+'</div><div class="lg-rar">'+s.r+'</div></div>';
    } else {
      grid += '<div class="lg-card locked"><div class="lg-emoji">🔒</div><div class="lg-name">???</div></div>';
    }
  });
  ...
  body.innerHTML =
    ...
    '<div class="lg-grid">'+grid+'</div>'+
    '<div class="lg-hint">'+(found<total ? (total-found)+' more hidden — find them on the pitch' : 'every story discovered — legendary')+'</div>'+
