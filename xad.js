...
function drawSquad(){
  ...
  if(!dice){ POOL=buildPool(); REROLL_USED=false; DEALT=true; POS_FILTER=null; TARGET_SLOT=null; render(); flash(""); const sx=el("s-xad"); if(sx) sx.scrollTop=0; return; }
  ROLLING = true;
  ...
  setTimeout(()=>{
    ...
    const sx = el("s-xad"); if(sx){ sx.scrollTop = 0; requestAnimationFrame(()=>{ sx.scrollTop = 0; }); }
...
function lockXI(){
  ...
  // podtytuł dwuczłonowy: [tytuł z puli] + [drugi człon = detectAlso z RESZTY składu]
  const capFirst = s2 => s2 ? s2.charAt(0).toUpperCase()+s2.slice(1) : s2;
  if(det.main){
    const tl = narrTagline(det.main) || NARRATIVES[det.main].desc || "";
    if(tl) html += '<div class="xad-flavor">'+tl+'</div>';
    if(also) html += '<div class="xad-flavor-rest">'+capFirst(also.clause)+'.</div>';
  }else if(archetype){
    const tl = archTagline(archetype) || "";
    if(tl) html += '<div class="xad-flavor">'+tl+'</div>';
    if(also) html += '<div class="xad-flavor-rest">'+capFirst(also.clause)+'.</div>';
    else { const rest = interpretRest(xiPlayers, null); if(rest) html += '<div class="xad-flavor-rest">'+rest+'</div>'; }
  }else{
    const tl = narrTagline("mixed") || interpretRest(xiPlayers, null) || eraDescriptor(xiPlayers);
    html += '<div class="xad-flavor">'+tl+'</div>';
    const mr = mixedRest(xiPlayers);
    if(mr) html += '<div class="xad-flavor-rest">'+capFirst(mr)+'.</div>';
  }
  ...
  try{
    ...
    LAST_SHARE = {
      score: score,
      formation: FORMATION,
      story: (typeof window.xadStoryFor==="function") ? window.xadStoryFor(xiPlayers) : null,
      slots: SLOTS.map(function(s){
        if(!s.player) return null;
        var p=s.player;
        return {x:s.x, y:s.y, pos:s.pos, grp:s.p, name:lastName(p.n), era:p.e, color:cardColor(p), shirt66:(is66 && core66.has(p.n))};
      })
    };
  }catch(e){}
...
function xadBuildCard(cb){
  ...
  FL.frame(ctx,{ kicker:"X I   A F T E R   D A R K", title:String(S.score), cta:"CAN YOU BEAT THIS?", url:"footylegendsquiz.co.uk" });
  FL.drawPitch(ctx);
  var ERA_CARD={C:"#c060ff",G:"#ffb030",M:"#30c0ff"};
  ...
  function draw(haveImg){
    S.slots.forEach(function(s){
      ...
      if(s.shirt66 && haveImg){
        ...
        ctx.beginPath(); ctx.arc(cx,cy-8,30,0,Math.PI*2); ctx.fillStyle="rgba(0,0,0,0.55)"; ctx.fill();
        ctx.lineWidth=3; ctx.strokeStyle=s.color; ctx.stroke(); ctx.restore();
        ctx.fillStyle=s.color; ctx.font="bold 20px Arial, sans-serif"; ctx.textAlign="center"; ctx.textBaseline="middle";
        ctx.fillText(s.pos, cx, cy-8); ctx.textBaseline="alphabetic";
      }
      ...
      ctx.fillStyle=(s.color||"#ffffff"); ctx.textAlign="center"; ctx.fillText(nm, cx, cy+42);
    });
    FL.drawCaption(ctx, S.story);
...
function xadShareCard(){ xadBuildCard(function(blob){ window.FLCard.shareBlob(blob, "xi-after-dark.png", "Can you beat "+(LAST_SHARE?LAST_SHARE.score:"")+"? footylegendsquiz.co.uk"); }); }
...
window.FLCard = (function(){
  var W=1080, H=1350, PITCH={x:130,y:248,w:820,h:690};
  ...
  function frame(ctx,opts){
    opts=opts||{};
    var kicker=opts.kicker||"F O O T Y   L E G E N D S   C A R D", title=opts.title||"", cta=opts.cta||"", url=opts.url||"footylegendsquiz.co.uk";
    var bg=ctx.createLinearGradient(0,0,0,H); bg.addColorStop(0,"#0a1f0c"); bg.addColorStop(1,"#051206");
    ...
    var lsg=ctx.createLinearGradient(0,0,W,H*0.5); lsg.addColorStop(0,"rgba(130,210,150,0.10)"); lsg.addColorStop(0.55,"rgba(130,210,150,0)");
    ctx.fillStyle=lsg; ctx.fillRect(0,0,W,H);
    var vg=ctx.createRadialGradient(W/2,H*0.42,200,W/2,H*0.42,H*0.72); vg.addColorStop(0,"rgba(0,0,0,0)"); vg.addColorStop(1,"rgba(0,0,0,0.55)");
    ctx.fillStyle=vg; ctx.fillRect(0,0,W,H);
    var M=34;
    ctx.save(); ctx.shadowColor="rgba(0,0,0,0.55)"; ctx.shadowBlur=40; ctx.shadowOffsetY=10;
    var pg=ctx.createLinearGradient(0,M,0,H-M); pg.addColorStop(0,"#103016"); pg.addColorStop(1,"#0a2210");
    ctx.fillStyle=pg; roundRect(ctx,M,M,W-2*M,H-2*M,26); ctx.fill(); ctx.restore();
    ctx.strokeStyle="#c9a73a"; ctx.lineWidth=4; roundRect(ctx,M,M,W-2*M,H-2*M,26); ctx.stroke();
    ctx.strokeStyle="rgba(232,200,96,0.5)"; ctx.lineWidth=1.5; roundRect(ctx,M+10,M+10,W-2*M-20,H-2*M-20,20); ctx.stroke();
    ctx.textAlign="center";
    ctx.fillStyle="#c9a73a"; ctx.font="bold 26px Arial, sans-serif"; ctx.fillText(kicker, W/2, 112);
    ctx.fillStyle="#ffffff"; var hs=66; ctx.font="900 "+hs+"px Arial, sans-serif";
    ...
    if(cta){ ctx.fillStyle="#ffffff"; ctx.font="bold 32px Arial, sans-serif"; ctx.fillText(cta, W/2, H-116); }
    ctx.fillStyle="#c9a73a"; ctx.font="bold 26px Arial, sans-serif"; ctx.fillText(url, W/2, H-76);
  }
  function drawPitch(ctx){
    ...
    var pgr=ctx.createRadialGradient(pX+pW/2,pY+pH*0.32,40,pX+pW/2,pY+pH*0.32,pW*0.9); pgr.addColorStop(0,"#1c5417"); pgr.addColorStop(1,"#0c2c0c");
    ctx.fillStyle=pgr; roundRect(ctx,pX,pY,pW,pH,14); ctx.fill(); ctx.restore();
    ctx.save(); roundRect(ctx,pX,pY,pW,pH,14); ctx.clip();
    for(var i=0;i<10;i++){ ctx.fillStyle = i%2 ? "rgba(255,255,255,0.025)":"rgba(0,0,0,0.04)"; ctx.fillRect(pX,pY+i*(pH/10),pW,pH/10); }
    ...
    var bxW=pW*0.5,bxH=pH*0.16; ctx.strokeRect(pX+(pW-bxW)/2,pY+pH-bxH-8,bxW,bxH);
    var syW=pW*0.24,syH=pH*0.07; ctx.strokeRect(pX+(pW-syW)/2,pY+pH-syH-8,syW,syH);
    ctx.restore();
  }
  ...
  function shareBlob(blob, fname, text){ fname=fname||"footy-legends-xi.png";
    try{ var file=new File([blob],fname,{type:"image/png"}); var data={files:[file], title:"Footy Legends", text:text||"footylegendsquiz.co.uk"};
      if(navigator.canShare && navigator.canShare({files:[file]}) && navigator.share){ navigator.share(data).catch(function(){}); return; } }catch(e){}
    saveBlob(blob, fname);
  }
  return { W:W, H:H, PITCH:PITCH, newCanvas:newCanvas, roundRect:roundRect, wrap:wrap, frame:frame, drawPitch:drawPitch, drawCaption:drawCaption, shareBlob:shareBlob, saveBlob:saveBlob };
})();
