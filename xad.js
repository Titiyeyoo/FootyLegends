...
function flRow(k,v){ return '<div class="legend-row"><span class="lk">'+k+'</span><span class="lv">'+v+'</span></div>'; }
var FL_TYPE = {
...
window.openLegend = function(){
  ...
  var shareBlock = ty
    ? '<button onclick="shareLegend()" style="font-family:inherit;margin-top:16px;border:1px solid #e9c46a;color:#e9c46a;background:transparent;border-radius:6px;font-size:11px;letter-spacing:1px;padding:11px 24px;cursor:pointer">SHARE MY DNA</button>'
  ...
  b.innerHTML=
    ...
    typeBlock+
    '<div class="legend-rows">'+facts+'</div>'+
    shareBlock;
  go("s-legend");
};
...
function flLegendShareLine(d){
  if(!d||d.games<5) return "What's your footy DNA? Find out on Footy Legends.";
  var ax=d.life, top="", tv=-1e9;
  var nm={vintage:"old-school",modern:"modern",stars:"a galactico picker",deep:"a deep-cut scout",loyalty:"a club loyalist"};
  for(var k in nm){ if(ax[k]>tv){tv=ax[k];top=k;} }
  var lead=(tv>6)?nm[top]:"a balanced picker";
  return "My footy DNA: "+d.favEra+"-era, "+lead+". What's yours?";
}
