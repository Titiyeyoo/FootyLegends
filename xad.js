...
try{
  ...
  window.FL_NARRATIVES = NARRATIVES;   // listy graczy — szablony w MY XI (NEW XI → Collections)
...
function flLoad(){
  ...
  return {v:1,games:0,
    ...
    mem:{lastSeen:{},lastSeenEra:{},lastSeenClub:{},lastSeenStory:{},formCount:{},shown:{}},
...
// Progi typu kibica. Byly 1.0/0.8 — za wysokie: gracz grajacy 40 gier czysto Modern
// nadal dostawal ALL-ROUNDER (przewaga +0.83 < 1.0). Pomiar: bez gustu -0.17, lekki gust +0.67.
// 0.6/0.5 rozdziela ich czysto i nadal NIE rozdaje typow graczom bez preferencji.
var FL_TYPE_MARGIN  = 0.6;   // typ glowny
var FL_TYPE_MARGIN2 = 0.5;   // typ drugorzedny
...
function flFanType(pr){
  ...
  var a=flAvgCounts(pr), best="balanced", bv=FL_TYPE_MARGIN; // margines nad norma na szczytowej osi
...
function flFanTypePair(pr){
  ...
  var primary = (arr[0] && arr[0].ex > FL_TYPE_MARGIN) ? arr[0].k : "balanced";
  var secondary = (primary!=="balanced" && arr[1] && arr[1].ex > FL_TYPE_MARGIN2) ? arr[1].k : null;
  return {
    primary: primary,
    secondary: secondary,
...
function logProfile(xiPlayers, pool, storyKey){
  try{
    ...
    // ── WARSTWA PAMIECI (Behaviour Collection) ──
    // Zapisujemy TYLKO to, czego nie da sie odtworzyc pozniej: KIEDY cos ostatnio wystapilo.
    // Nic nie pokazuje. Silnik obserwacji przyjdzie pozniej i bedzie to czytal.
    try{
      var g = pr.games;
      if(!pr.mem) pr.mem = {};
      var M = pr.mem;
      if(!M.lastSeen)      M.lastSeen = {};
      if(!M.lastSeenEra)   M.lastSeenEra = {};
      if(!M.lastSeenClub)  M.lastSeenClub = {};
      if(!M.lastSeenStory) M.lastSeenStory = {};
      if(!M.formCount)     M.formCount = {};
      if(!M.shown)         M.shown = {};
      // gracze + ery + kluby z tego XI
      var seenEra = {}, seenClub = {};
      for(var m=0;m<xiPlayers.length;m++){
        var pl = xiPlayers[m]; if(!pl) continue;
        M.lastSeen[pl.n] = g;
        if(pl.e) seenEra[pl.e] = 1;
        if(pl.c && pl.c.length) seenClub[pl.c[0]] = 1;
      }
      for(var ek in seenEra)  M.lastSeenEra[ek]  = g;
      for(var ck in seenClub) M.lastSeenClub[ck] = g;
      if(storyKey) M.lastSeenStory[storyKey] = g;
      // formacja (After Dark ma 5 do wyboru)
      var _f = (typeof FORMATION!=="undefined" && FORMATION) ? FORMATION : null;
      if(_f){ M.formCount[_f] = (M.formCount[_f]||0)+1; M.lastForm = _f; }
    }catch(e){}
...
/* ═══════════════════════════════════════════════════════════════════
   BEHAVIOUR COLLECTION — silnik obserwacji
   Zasady: interpretacja + dowod liczbowy. Nigdy sucha statystyka.
   Czysta funkcja: czyta profil, NIC nie zapisuje (poza shown, robi to UI).
   ═══════════════════════════════════════════════════════════════════ */
var FL_OBS_MIN_GAMES = 15;   // ponizej — cisza

// progi
function _obsRetP(g){ return Math.max(15, Math.round(g*0.25)); }  // gracz
function _obsRetX(g){ return Math.max(10, Math.round(g*0.15)); }  // era / klub / narracja

var FL_ERA_NAME = {C:"Classic", G:"Golden", M:"Modern"};

// ile z ostatnich N gier spelnia warunek (recent[] trzyma do 30)
function _obsRecent(pr, idx, min){
  var r=pr.recent||[], n=r.length, hit=0;
  if(n<10) return null;
  for(var i=0;i<n;i++){ if(r[i].pk && r[i].pk[idx]>=min) hit++; }
  return {hit:hit, n:n, pct:Math.round(100*hit/n)};
}
function _cap(s){ return String(s||"").replace(/\b\w/g, function(c){return c.toUpperCase();}); }

function flObservations(pr){
  var out=[];
  try{
    if(!pr) return out;
    var g=pr.games||0, M=pr.mem||{}, fav=pr.favPlayers||{}, st=pr.stories||{};
    var LS=M.lastSeen||{}, LSE=M.lastSeenEra||{}, LSC=M.lastSeenClub||{}, LST=M.lastSeenStory||{}, FC=M.formCount||{};
    var IDX={C:0,G:1,M:2,stars:3,deep:4,attack:5,defense:6};
    // FIRST/MOMENT: dzialaja od pierwszej gry (to zdarzenia, nie wzorce).
    // HABIT/RETURN: potrzebuja historii — dopiero od FL_OBS_MIN_GAMES.
    var mature = (g >= FL_OBS_MIN_GAMES);

    if(mature){
    /* ── RETURN — powroty (wymaga pamieci; zadziala gdy dojrzeje) ── */
    var pThr=_obsRetP(g);
    for(var pn in LS){
      var gap = g - LS[pn];
      if(gap>=pThr && fav[pn]>=2){
        out.push({id:"ret_p_"+pn, cat:"RETURN", w:100+gap,
          text:pn+" returned.", proof:"After "+gap+" lineups away."});
      }
    }
    var xThr=_obsRetX(g);
    for(var ek in LSE){
      var eg = g - LSE[ek];
      if(eg>=xThr) out.push({id:"ret_e_"+ek, cat:"RETURN", w:80+eg,
        text:"The "+(FL_ERA_NAME[ek]||ek)+" era returned to your XI.", proof:"First time in "+eg+" lineups."});
    }
    for(var ck in LSC){
      var cg = g - LSC[ck];
      if(cg>=xThr) out.push({id:"ret_c_"+ck, cat:"RETURN", w:75+cg,
        text:_cap(ck)+" are back in your side.", proof:"After "+cg+" lineups without them."});
    }
    for(var sk in LST){
      var sg = g - LST[sk];
      if(sg>=xThr && NARRATIVES[sk]) out.push({id:"ret_s_"+sk, cat:"RETURN", w:85+sg,
        text:NARRATIVES[sk].name+" resurfaced.", proof:"First time in "+sg+" lineups."});
    }

    /* ── HABIT — nawyki (dziala od reki na starych profilach) ── */
    for(var hp in fav){
      var c=fav[hp], pct=Math.round(100*c/g);
      if(c>=8 && pct>=30){
        out.push({id:"hab_p_"+hp, cat:"HABIT", w:40+pct,
          text:hp+" keeps finding his way back.", proof:c+" appearances — "+pct+"% of your lineups."});
      }
    }
    var eraTxt={C:"You keep reaching for the Classic era.",G:"The Golden era runs through your teams.",M:"Modern football is taking over your XI."};
    ["C","G","M"].forEach(function(e){
      var r=_obsRecent(pr, IDX[e], 6);
      if(r && r.pct>=60) out.push({id:"hab_era_"+e, cat:"HABIT", w:30+r.pct,
        text:eraTxt[e], proof:r.pct+"% of your recent lineups."});
    });
    var rD=_obsRecent(pr, IDX.deep, 3);
    if(rD && rD.pct>=60) out.push({id:"hab_deep", cat:"HABIT", w:35+rD.pct,
      text:"You've started exploring forgotten legends.", proof:"Deep cuts in "+rD.hit+" of your last "+rD.n+"."});
    var rS=_obsRecent(pr, IDX.stars, 3);
    if(rS && rS.pct>=60) out.push({id:"hab_stars", cat:"HABIT", w:35+rS.pct,
      text:"The big names always make your XI.", proof:"Stars in "+rS.hit+" of your last "+rS.n+"."});
    // klub, ktory ciagle wraca (lt = najwiekszy klub w danym XI)
    var clubHit=0, rN=(pr.recent||[]).length;
    for(var ci=0;ci<rN;ci++){ if((pr.recent[ci].lt||0)>=3) clubHit++; }
    if(rN>=10 && Math.round(100*clubHit/rN)>=50){
      out.push({id:"hab_clubcore", cat:"HABIT", w:36+Math.round(100*clubHit/rN),
        text:"You keep building around one club.", proof:"3+ teammates from the same side in "+clubHit+" of your last "+rN+"."});
    }
    // przeciwienstwo gwiazd — swiadome unikanie oczywistych nazwisk
    var rNS=_obsRecent(pr, IDX.stars, 1);
    if(rNS && rNS.pct<=20 && rNS.n>=15){
      out.push({id:"hab_nostars", cat:"HABIT", w:44,
        text:"You don't need the obvious names.", proof:"Barely a star in your last "+rNS.n+" lineups."});
    }
    // ilu graczy weszlo w polowe Twoich skladow
    var loyalN=0; for(var lp in fav){ if(fav[lp] >= g*0.5) loyalN++; }
    if(loyalN>=4 && g>=25){
      out.push({id:"hab_core", cat:"HABIT", w:46,
        text:"Some legends never stay away for long.", proof:loyalN+" players appear in over half your lineups."});
    }
    // UWAGA: NIE robimy obserwacji z attack/defense.
    // Liczba obroncow/napastnikow wynika z FORMACJI, nie z wyboru gracza:
    // 4-4-2 i 4-3-3 maja 5 obroncow, 4-2-3-1 ma 7 — prog ">=5" strzelalby ZAWSZE.
    // To nie jest sygnal o graczu. Formacja (ponizej) juz jest wyborem — i ona wystarcza.
    // formacja
    var fTot=0, fTop=null, fTopN=0;
    for(var fk in FC){ fTot+=FC[fk]; if(FC[fk]>fTopN){ fTopN=FC[fk]; fTop=fk; } }
    if(fTop && fTot>=10){
      var fPct=Math.round(100*fTopN/fTot);
      if(fPct>=60) out.push({id:"hab_form_"+fTop, cat:"HABIT", w:38+fPct,
        text:fTop+" is becoming your shape.", proof:fPct+"% of your lineups."});
    }

    }  /* koniec HABIT/RETURN (wymagaja >= FL_OBS_MIN_GAMES) */

    // nigdy nie zmieniasz ksztaltu — albo zmieniasz ciagle
    if(fTop && fTot>=15){
      var fPct2=Math.round(100*fTopN/fTot);
      if(fPct2>=85) out.push({id:"hab_loyalform", cat:"HABIT", w:52,
        text:"You've found your shape and you're not letting go.", proof:fTop+" in "+fPct2+"% of your lineups."});
      var fUsed=0; for(var fu in FC) fUsed++;
      if(fUsed>=4 && fPct2<=40) out.push({id:"hab_shapeshift", cat:"HABIT", w:48,
        text:"You never settle on one shape.", proof:fUsed+" different formations used."});
    }

    /* ── MOMENT — chwile (jednorazowe, najmocniejsza kategoria) ── */
    // XI z jednej ery — trudne i rzadkie
    var pureEra=null, pureN=0;
    for(var mi=0;mi<(pr.recent||[]).length;mi++){
      var rr=pr.recent[mi];
      ["C","G","M"].forEach(function(e){ if(rr.pk && rr.pk[IDX[e]]>=11){ pureEra=e; pureN++; } });
    }
    if(pureEra) out.push({id:"mom_pure_"+pureEra, cat:"MOMENT", w:170,
      text:"An XI from a single era.", proof:"Eleven names, all "+(FL_ERA_NAME[pureEra]||pureEra)+"."+(pureN>1?" You've done it "+pureN+" times.":"")});
    // XI bez ani jednej gwiazdy
    var noStarN=0;
    for(var mj=0;mj<(pr.recent||[]).length;mj++){ if(pr.recent[mj].pk && pr.recent[mj].pk[IDX.stars]===0) noStarN++; }
    if(noStarN) out.push({id:"mom_nostar", cat:"MOMENT", w:155,
      text:"You built an XI without a single star.", proof:noStarN>1?("You've done it "+noStarN+" times."):"Not one obvious name."});
    // klub, ktory zdominowal jeden sklad (5+ z jednej druzyny)
    var domN=0;
    for(var mk=0;mk<(pr.recent||[]).length;mk++){ if((pr.recent[mk].lt||0)>=5) domN++; }
    if(domN) out.push({id:"mom_clubdom", cat:"MOMENT", w:160,
      text:"One club took over your XI.", proof:"Five or more from the same side."+(domN>1?" "+domN+" times.":"")});
    // wiele historii z tego samego skladu
    var deepDig=0;
    for(var ml=0;ml<(pr.recent||[]).length;ml++){ if(pr.recent[ml].pk && pr.recent[ml].pk[IDX.deep]>=8) deepDig++; }
    if(deepDig) out.push({id:"mom_deepxi", cat:"MOMENT", w:165,
      text:"An XI built almost entirely from forgotten names.", proof:"Eight or more deep cuts in one lineup."});

    /* ── FIRST / MOMENT — jednorazowe zdarzenia, dzialaja OD PIERWSZEJ GRY ── */
    // zrodlo prawdy takie samo jak Legacy: FL_XAD_STORIES (17 narracji + 3 archetypy = 20)
    var totStories = 0, nStories = 0;
    try{
      var _all = (typeof window!=="undefined" && window.FL_XAD_STORIES) ? window.FL_XAD_STORIES : [];
      totStories = _all.length;
      var _disc = new Set(JSON.parse(localStorage.getItem("fl_xad_stories")||"[]"));
      for(var a2=0;a2<_all.length;a2++){ if(_disc.has(_all[a2].k)) nStories++; }
    }catch(e){}
    if(nStories>=1)  out.push({id:"first_story", cat:"FIRST", w:22, text:"You discovered your first story.", proof:"The book has begun."});
    if(nStories>=10) out.push({id:"first_10", cat:"FIRST", w:28, text:"Ten stories in.", proof:"Your book is filling up."});
    if(totStories && nStories>=totStories) out.push({id:"first_all", cat:"FIRST", w:55, text:"Every story discovered.", proof:"All "+totStories+". Legendary."});
    // legendy wystawione w XI — progi schodkowe, tekst zgodny z liczba
    var distinct=0; for(var d2 in fav) distinct++;
    var dbTot = (typeof DB!=="undefined" && DB.length) ? DB.length : 804;
    if(distinct>=500)      out.push({id:"first_500leg", cat:"FIRST", w:40, text:"Five hundred legends have worn your shirt.", proof:distinct+" of "+dbTot+"."});
    else if(distinct>=250) out.push({id:"first_250leg", cat:"FIRST", w:34, text:"Two hundred and fifty legends have worn your shirt.", proof:distinct+" of "+dbTot+"."});
    else if(distinct>=100) out.push({id:"first_100leg", cat:"FIRST", w:30, text:"A hundred different legends have worn your shirt.", proof:distinct+" of "+dbTot+"."});
    // MOMENT — najczesciej odtwarzana narracja
    var sTop=null, sTopN=0;
    for(var s3 in st){ if(st[s3]>sTopN){ sTopN=st[s3]; sTop=s3; } }
    if(sTop && sTopN>=3 && NARRATIVES[sTop]) out.push({id:"mom_top_"+sTop, cat:"MOMENT", w:60+sTopN,
      text:NARRATIVES[sTop].name+" keeps coming back to life.", proof:"You've rebuilt it "+sTopN+" times."});

    out.sort(function(a,b){ return b.w-a.w; });
  }catch(e){}
  return out;
}
window.flGetObservations = function(){
  try{ return flObservations(flLoad()); }catch(e){ return []; }
};
...
var FL_UI = {
  ...
  // "27:30" wygladalo jak godzina, a to byl czas gry. Teraz mowi wprost, co to jest.
  time: function(ms){
    var s = Math.floor((ms||0)/1000);
    if(s < 60)    return s+"s played";
    if(s < 3600)  return Math.floor(s/60)+"m played";
    if(s < 86400){
      var h=Math.floor(s/3600), m=Math.floor((s%3600)/60);
      return m>0 ? (h+"h "+m+"m played") : (h+"h played");
    }
    var d=Math.floor(s/86400), hh=Math.floor((s%86400)/3600);
    return hh>0 ? (d+"d "+hh+"h played") : (d+"d played");
  }
};
...
window.openLegend = function(silent){
  ...
  var facts=flRow("ERA", d.games<5?"\u2014":d.favEra)
          ...
          + flRow("LEGENDS FIELDED", d.usedPlayers+" / "+d.dbTotal)
...
function xadShareXI(){
  ...
  const bxW=pW*0.59,bxH=pH*0.17; ctx.strokeRect(pX+(pW-bxW)/2,pY+pH-bxH,bxW,bxH);
  // polkole ("D") nad polem karnym — brakowalo
  ctx.beginPath(); ctx.arc(pX+pW/2, pY+pH-bxH, pW*0.16, Math.PI, 0); ctx.stroke();
