...
function render(){
  ...
  // PITCH — boisko teletext, sloty pozycjonowane absolutnie wg x/y formacji
  ...
  SLOTS.forEach((s,i) => {
    ...
    if(s.player){
      node.className = "xad-pos filled e-"+s.player.e;
      node.innerHTML = '<span class="pn">'+escXad(lastName(s.player.n))+'</span>'+
                       '<span class="pc">'+escXad(clubLabel(s.player))+'</span>';
      node.onclick = () => removeFromSlot(i);
    }else{
      node.className = "xad-pos empty";
      node.innerHTML = '<span class="pl">'+s.pos+'</span>';
    }
...
// na boisku pokazujemy nazwisko (krótko); pełne imię i tak jest w puli i w reveal
