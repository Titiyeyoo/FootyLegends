...
window.xadStoryFor = function(xiPlayers){
  ...
  if(det && det.main){
    ...
    return {key:det.main, name:N.name, emoji:N.emoji, desc:N.desc||""};
  }
  const a = detectArchetype(xiPlayers);
  if(a) return {key:"arch_"+a.kind, name:a.name, emoji:a.emoji, desc:""};
  return {key:"mixed", name:"MIXED XI", emoji:"🌍", desc:"Your own blend — no single story."};
};
function xadShareXI(){
