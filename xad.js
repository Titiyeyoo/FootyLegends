...
// klik "XI AFTER DARK" w menu → ekran intro (nic jeszcze nie losujemy)
function openIntro(){
  showXadScreen("xad-intro");
}
// "WYLOSUJ MOICH 30" → losuj pulę, ustaw 4-4-2, od razu w draft (BAM)
function drawSquad(){
  POOL = buildPool();
  REROLL_USED = false;
  FORMATION = "4-4-2";
...
function placePlayer(p){
  ...
  slot.player = p; flash(""); render(true, p.n);          // animuj wskoczenie tego zawodnika
}
...
function render(animate, justJumped){
  // POOL (lista nazwisk — pula to wybór, koła są na boisku)
...
function lockXI(){
  ...
  ["GK","DF","MF","FW"].forEach(grp=>{      // od bramkarza w górę (czyta się jak skład)
...
function showXadScreen(id){
  ["xad-intro","xad-draft","xad-reveal"].forEach(s=>{
