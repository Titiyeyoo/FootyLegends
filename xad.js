...
function lockXI(){
  ...
  if(det.main){
    ...
    // interpretacja reszty składu (zmienna co grę) — zastępuje sztywny "with X backbone"
    const rest = interpretRest(xiPlayers, det.main);
    if(rest) html += '<div class="xad-flavor">'+rest+'</div>';
  }else{
    // MIXED XI — opisujemy cały skład żywą interpretacją zamiast suchego "Generational Mix"
  ...
  // dominująca era w XI (do glow ery na nie-narracyjnych)
  ...
  SLOTS.forEach(s=>{
    const p=s.player;
    // warstwa 1: rdzeń wykrytej narracji → złoty perfect glow
