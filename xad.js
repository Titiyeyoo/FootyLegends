...
const CFG = {
  ...
  POOL_SIZE: 30,
  POOL_QUOTA:  { GK:3, DF:10, MF:11, FW:6 },   // target struktury puli
  FORMATIONS: {
...
function buildPool(){
  ...
  for (const f in CFG.FORMATIONS)
    for (const pos in need) need[pos] = Math.max(need[pos], CFG.FORMATIONS[f][pos]);
...
function buildSlots(){
  SLOTS = [];
  const F = CFG.FORMATIONS[FORMATION];
  for(let i=0;i<F.GK;i++) SLOTS.push({pos:"GK",player:null});
  for(let i=0;i<F.DF;i++) SLOTS.push({pos:"DF",player:null});
  for(let i=0;i<F.MF;i++) SLOTS.push({pos:"MF",player:null});
  for(let i=0;i<F.FW;i++) SLOTS.push({pos:"FW",player:null});
}
...
function changeFormation(){
  ...
  buildSlots();
  for(const p of kept){
    const slot = SLOTS.find(s=>s.pos===p.p && !s.player);
...
function placePlayer(p){
  if(inXI(p.n)) return;
  const slot = SLOTS.find(s=>s.pos===p.p && !s.player);
  if(!slot){ flash("NO "+p.p+" SLOTS IN "+FORMATION); return; }
  slot.player = p; flash(""); render();
}
...
function renderFormationGrid(){
  const g = el("xad-formation-grid"); if(!g) return;
  g.innerHTML = "";
  Object.keys(CFG.FORMATIONS).forEach(f => {
    const d = document.createElement("div");
    d.className = "xad-formcard";
    d.innerHTML = '<span class="fname">'+f+'</span><span class="fsub">'+(FORM_HINT[f]||"")+'</span>';
    d.onclick = () => { FORMATION = f; (SLOTS.some(s=>s.player) ? changeFormation() : startDraft()); };
    g.appendChild(d);
  });
}
...
function render(){
  ...
  POOL.forEach(p => {
    const d = document.createElement("div");
    d.className = "xad-card e-"+p.e + (inXI(p.n) ? " used":"");
    d.innerHTML = '<span class="nm">'+escXad(p.n)+'</span>'+
                  '<span class="meta">'+escXad(clubLabel(p))+'</span>'+
                  '<span class="pos">'+posLabel(p)+'</span>';   // FIX: pos, nie p
    if(!inXI(p.n)) d.onclick = () => placePlayer(p);
    poolEl.appendChild(d);
  });
  // pitch (FW top → GK bottom)
  const pitchEl = el("xad-pitch"); pitchEl.innerHTML = "";
  const starts = {}; ["GK","DF","MF","FW"].forEach(pos=>{ starts[pos]=SLOTS.findIndex(s=>s.pos===pos); });
  ["FW","MF","DF","GK"].forEach(pos=>{
    const n = SLOTS.filter(s=>s.pos===pos).length; if(!n) return;
    const lineEl = document.createElement("div"); lineEl.className = "xad-line";
    for(let k=0;k<n;k++){
      const i = starts[pos]+k; const s = SLOTS[i];
      const cell = document.createElement("div");
      if(s.player){
        cell.className = "xad-slot filled e-"+s.player.e;
        cell.innerHTML = '<span class="sn">'+escXad(s.player.n)+'</span><span class="sc">'+escXad(clubLabel(s.player))+' · '+posLabel(s.player)+'</span>';
        cell.onclick = () => removeFromSlot(i);
      }else{
        cell.className = "xad-slot";
        cell.innerHTML = '<span class="ph">'+pos+'</span>';
      }
      lineEl.appendChild(cell);
    }
    pitchEl.appendChild(lineEl);
  });
...
function lockXI(){
  ...
  ["FW","MF","DF","GK"].forEach(pos=>{
    SLOTS.filter(s=>s.pos===pos).forEach(s=>{
      const p=s.player;
      html += '<div class="xad-row e-'+p.e+'"><span>'+escXad(p.n)+'</span><span class="rp">'+escXad(clubLabel(p))+' · '+posLabel(p)+'</span></div>';
    });
  });
  html += '</div>';
...
function showXadScreen(id){
  ["xad-formation","xad-draft","xad-reveal"].forEach(s=>{
...
window.xadNewRound   = newRound;
window.xadLockXI     = lockXI;
window.xadChangeForm = () => showXadScreen("xad-formation");
