...
function buildRadarSVG(life, form){
  var cx=160, cy=150, R=108, ang=[-90,-18,54,126,198];
  ...
  var svg='<svg viewBox="0 0 320 300" width="100%" xmlns="http://www.w3.org/2000/svg">';
  ...
  for(var j=0;j<5;j++){ var a2=ang[j]*Math.PI/180, lx=cx+(R+18)*Math.cos(a2), ly=cy+(R+18)*Math.sin(a2);
    ...
    svg+='<text x="'+lx.toFixed(1)+'" y="'+(ly+3).toFixed(1)+'" fill="#9a967f" font-size="9" text-anchor="'+anc+'">'+labels[j]+'</text>'; }
  return svg+'</svg>';
}
...
window.openLegend = function(){
  ...
  b.innerHTML=
    ...
    '<div class="legend-soon">'+gate+' &middot; your fan type unlocks later</div>'+
    '<button class="legend-share" onclick="shareLegend()">SHARE MY LEGEND</button>';
  go("s-legend");
};
...
window.shareLegend = function(){
  ...
  var url="https://footylegendsquiz.co.uk", txt="My Footy Legend on Footy Legends - see your XI's DNA. "+url;
  try{ if(navigator.share){ navigator.share({text:txt,url:url}); return; } }catch(e){}
  try{ navigator.clipboard.writeText(txt); if(typeof toast==="function") toast("Link copied",""); }catch(e){}
};
