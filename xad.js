...
function detectArchetype(xiPlayers){
  ...
  if(topClubN >= 5){
    ...
    return {name: tpl(clubDisplay(topClub)), emoji: ARCH_EMOJI, kind:"club", key:topClub, count:topClubN};
  }
  if(topEraN >= 8){
    const arr = ARCH_ERA_NAMES[topEra];
    return {name: arr[rnd(arr.length)], emoji: ARCH_EMOJI, kind:"era", key:topEra, count:topEraN};
  }
  return null;
}
