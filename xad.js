...
function drawSquad(){
  ...
  setTimeout(()=>{
    ...
    render(); flash("");
    // kaskada: zawodnicy w puli pojawiają się jeden po drugim (szybko)
    cascadePool();
  }, 520);
}
// kaskada pojawiania kart w puli — dodaje klasę po kolei (CSS robi fade/slide)
function cascadePool(){
  const cards = el("xad-pool") ? el("xad-pool").querySelectorAll(".xad-card") : [];
  cards.forEach((c,i)=>{
    c.classList.add("dealt-in");
    c.style.animationDelay = (i*14)+"ms";       // ~0.4s na całą pulę 30
  });
}
...
/* ── INTERPRETACJA RESZTY SKŁADU ──────────────────────────────────────
   ...
   Banki sformułowań (1 z kilku losowany za każdym razem → świeżość bez nowych narracji): */
const REST_PHRASES = {
...
function pickPhrase(key){
  const arr = REST_PHRASES[key] || REST_PHRASES.chaos;
...
function interpretRest(xiPlayers, mainK){
  ...
  if(stars >= majority && stars >= 3) return pickPhrase("legends");
  if(domEraN >= majority && domEraN >= 3){
    return pickPhrase("era_"+domEra.toLowerCase());
  }
  return pickPhrase("chaos");
}
...
function detectArchetype(xiPlayers){
  ...
  // klub > era gdy oba; progi: klub>=5, era>=8
  if(topClubN >= 5){
...
function lockXI(){
  ...
  if(det.main){
    ...
    // brak narracji → spróbuj archetyp (Club Tribute / Same Era)
    archetype = detectArchetype(xiPlayers);
    if(archetype){
      html += '<div class="xad-headline">'+archetype.emoji+' '+archetype.name+'</div>';
      const rest = interpretRest(xiPlayers, null);
      if(rest) html += '<div class="xad-flavor">'+rest+'</div>';
    }else{
      // ani narracja, ani archetyp → MIXED z interpretacją reszty
  ...
  SLOTS.forEach(s=>{
    ...
    // warstwa 1: rdzeń narracji → złoty (link-narr), rdzeń archetypu → magenta (link-arch)
    // warstwa 2: reszta z dominującej ery → glow ery (tło spójności)
    let link = "";
    if(narrPlayers.has(p.n)) link = " link-narr";
    else if(archCore(p)) link = " link-arch";
    else if(p.e===domEra && eraCount[domEra]>=5) link = " link-era era-"+p.e.toLowerCase();
...
function setupStickyPitch(){
  ...
  const THRESHOLD = 50;   // px scrolla zanim boisko się skurczy
  const onScroll = () => {
    const scroller = el("s-xad"); if(!scroller) return;
    const pitchCol = scroller.querySelector(".xad-col-pitch");
    if(!pitchCol) return;
    // przed PICK MY 30 (pusta pula) nie ma czego scrollować — boisko nie kurczy się, żeby nie skakało
    if(!DEALT){ pitchCol.classList.remove("shrunk"); return; }
    // scroll może być na oknie, <html>, <body> albo na #s-xad — bierzemy największy
    const top = Math.max(
      window.scrollY || 0,
