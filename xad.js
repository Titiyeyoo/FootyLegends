...
function flFootballLife(games){
  ...
  var mood = games<5?"EARLY DAYS":games<15?"FINDING YOUR VOICE":games<40?"MAKING YOUR NAME":games<80?"A SEASON IN":games<150?"SEASONED GAFFER":"A LIFETIME IN THE GAME";
  var tail = mins>=1 ? (" \u00b7 "+t) : "";
...
function placePlayer(p){
  ...
  slot.player = p; flash("");
  if(picked()===1 && !FL_BUILD_START) FL_BUILD_START = Date.now(); // pierwszy pick po rollu
...
window.openLegend = function(){
  ...
  b.innerHTML=
    ...
    '<div class="legend-title" style="margin-top:30px"><span onclick="try{changeName()}catch(e){}" style="cursor:pointer;border-bottom:2px dotted #7a7660">'+flLegendOwner()+'</span> FOOTY LEGEND</div>'+
  ...
  if(typeof _ccn === "function"){
    window.confirmChangeName = function(){
      ...
      try{
        ...
        if(s && s.classList.contains("on") && typeof window.openLegend==="function") window.openLegend();
      }catch(e){}
      return r;
    };
  }
