...
function xadBuildCard(cb){
  ...
  var sized = ENGLAND_SHIRT_SVG.replace("<svg ", '<svg width="108" height="108" ');
  var img=new Image();
  function draw(haveImg){
    S.slots.forEach(function(s){
      ...
      ctx.beginPath(); ctx.ellipse(cx, cy+32, 20, 5, 0, 0, Math.PI*2); ctx.fillStyle="rgba(0,0,0,0.55)"; ctx.fill(); ctx.restore();
      if(s.shirt66 && haveImg){
        ctx.drawImage(img, cx-52, cy-60, 104, 104);
      } else {
        var R=40, ccx=cx, ccy=cy-8;
        ...
        ctx.fillStyle="#ffffff"; ctx.font="bold 26px Arial, sans-serif"; ctx.textAlign="center"; ctx.textBaseline="middle";
        ctx.fillText(s.pos, ccx, ccy); ctx.textBaseline="alphabetic";
      }
      var eh=ERA_CARD[s.era]; if(eh){ ctx.beginPath(); ctx.arc(cx+28,cy-40,11,0,Math.PI*2); ctx.fillStyle=eh; ctx.fill(); ctx.lineWidth=2; ctx.strokeStyle="#000"; ctx.stroke(); }
      var nm=s.name, fs=26; ctx.font="bold "+fs+"px Arial, sans-serif";
      while(ctx.measureText(nm).width>130 && fs>14){ fs--; ctx.font="bold "+fs+"px Arial, sans-serif"; }
      ctx.textAlign="center"; ctx.lineJoin="round";
      // #2 ciemny kontur pod nazwiskiem (czytelność na każdym pasie)
      ctx.lineWidth=4; ctx.strokeStyle="rgba(0,0,0,0.85)"; ctx.strokeText(nm, cx, cy+52);
      ctx.fillStyle=(s.shirt66 ? "#ffffff" : st.pname); ctx.fillText(nm, cx, cy+52);
    });
...
function xadShareCard(){ xadBuildCard(function(blob){
...
window.FLCard = (function(){
  ...
  function drawRarityFooter(ctx,o){
    ...
    var found=0, tot=19; try{ found=getXadStories().size; tot=Object.keys(STORY_INFO).length; }catch(e){}
    var col = myth ? "#ff5a48" : tier==="legendary" ? "#ffd24a" : tier==="rare" ? "#cdd6df" : "#9a8c5a";
    var lbl = myth ? "MYTHIC DISCOVERY" : tier==="legendary" ? "LEGENDARY FIND" : tier==="rare" ? "RARE DISCOVERY" : "STORY FOUND";
    ctx.textAlign="center";
    if(ctx.letterSpacing!==undefined) ctx.letterSpacing="3px";
    ctx.fillStyle=col; ctx.font="bold 27px Arial, sans-serif"; ctx.fillText(lbl, W/2, H-240);
    ctx.fillStyle="#6f7d71"; ctx.font="bold 21px Arial, sans-serif"; ctx.fillText(found+" / "+tot+" DISCOVERED", W/2, H-214);
    if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
    ctx.textAlign="center";
  }
