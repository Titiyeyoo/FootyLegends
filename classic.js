...
(function(){
  ...
  function clBuildCard(cb){
    ...
    function drawPlayers(haveImg){
      ...
      xi.forEach(function(s){
        ...
        ctx.beginPath(); ctx.ellipse(cx, cy+30, 18, 5, 0, 0, Math.PI*2); ctx.fillStyle="rgba(0,0,0,0.55)"; ctx.fill(); ctx.restore();
        ...
        while(ctx.measureText(nm).width>132 && fs>17){ fs--; ctx.font="bold "+fs+"px Arial, sans-serif"; }
  ...
  window.clShareCard=function(){
    clBuildCard(function(blob){
      ...
      const data={files:[file], title:"My England XI", text:"Can you build a better XI? \ud83d\udc47", url:"https://footylegendsquiz.co.uk"};
