...
(function(){
  ...
  function clRenderList(){
    ...
    if(CL.target==null){ box.innerHTML=""; CL.sugg=[]; return; }
    const q=(($("cl-input")||{}).value||"").trim().toLowerCase();
    const eras=Object.keys(CL.fEra).filter(function(k){return CL.fEra[k];});
    const anyF = q || CL.fPos || CL.fClub || eras.length;
    if(!anyF){ box.innerHTML='<div class="cl-empty">search a name — or pick a filter (era / position / club)</div>'; CL.sugg=[]; return; }
