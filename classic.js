...
(function(){
  ...
  window.clSearch = function(v){
    ...
    let res=(window.DB||[]).filter(p => (p.p===slot.p || p.pos===slot.pos) && !picked.has(p.n) && p.n.toLowerCase().includes(q));
    ...
    box.innerHTML = CL.sugg.map((p,idx)=>
      '<div class="cl-sg" onclick="clPickIdx('+idx+')"><span class="cl-sg-n">'+p.n+'</span><span class="cl-sg-c">'+(p.pos||p.p)+' · '+clClub(p)+'</span></div>').join('');
  };
  ...
  function clBuildCard(cb){
    ...
    function drawPlayers(haveImg){
      ...
      xi.forEach(function(s){
        ...
        ctx.lineWidth=4; ctx.strokeStyle="rgba(0,0,0,0.85)"; ctx.strokeText(nm, cx, cy+shH/2+22);
        ctx.fillStyle="#ffffff"; ctx.fillText(nm, cx, cy+shH/2+22);
      });
