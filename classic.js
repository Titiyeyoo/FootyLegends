...
(function(){
  ...
  const COLG = {GK:"#ffdd00",DF:"#88ff44",MF:"#ff6600",FW:"#ff2200"};
  ...
  window.openClassic = function(){
    clBuildSlots(); CL.target=null; CL.sugg=[];
    if(typeof go==="function") go("s-classic");
    clRender();
  };

  function clChangeFormation(f){ if(!CL_FORMATIONS[f]) return; CL.formation=f; clBuildSlots(); CL.target=null; clRender(); }
  ...
  function clRender(){
    ...
    // pitch
    if($("cl-pitch")) $("cl-pitch").innerHTML = CL.slots.map((s,i)=>{
      const col = COLG[s.p]||"#aaa", sel=(CL.target===i);
      const pos = "left:"+(s.x*100)+"%;top:"+(s.y*100)+"%";
      if(s.player){
        return '<div class="cl-slot'+(sel?' sel':'')+'" style="'+pos+'" onclick="clTap('+i+')">'+
          '<div class="cl-dot" style="border-color:'+col+';color:'+col+'">'+s.pos+'</div>'+
          '<div class="cl-nm">'+clLast(s.player.n)+'</div></div>';
      }
      return '<div class="cl-slot empty'+(sel?' sel':'')+'" style="'+pos+'" onclick="clTap('+i+')">'+
        '<div class="cl-dot empty" style="border-color:'+col+';color:'+col+'">'+s.pos+'</div></div>';
    }).join('');
    // search panel + done
    renderSearch();
  ...
  window.clTap = function(i){
    const s=CL.slots[i];
    if(s.player){ s.player=null; CL.target=null; CL.sugg=[]; clRender(); return; } // tap filled = clear
    CL.target = (CL.target===i)?null:i; CL.sugg=[]; clRender();
    if(CL.target!=null){ const inp=$("cl-input"); if(inp){ inp.value=""; setTimeout(()=>inp.focus(),40); } }
  };
  ...
  function renderSearch(){
    ...
    if($("cl-search-label")) $("cl-search-label").textContent = "Add "+CL.slots[CL.target].pos+" — type a name";
  }
