...
function setupBackButton(){
  ...
  window.addEventListener('popstate', function(e){
    ...
    // Toast on every popstate so we can see what's happening on real device
    if(typeof toast === 'function') toast('BACK PRESSED', '');

    try {
      // 1. Close any visible overlay first
      ...
      for(const ovId of overlays){
        ...
        if(ov && ov.style.display && ov.style.display !== 'none'){
          ...
          if(typeof toast === 'function') toast('CLOSED OVERLAY', '');
          // Re-push — next BACK fires popstate again
          try { history.pushState({screen: 'app'}, '', ''); } catch(e){}
          isHandlingPopstate = false;
          return;
        }
      }

      // 2. Find current screen
      const visibleScreen = document.querySelector('.screen.on');
      const currentId = visibleScreen ? visibleScreen.id : null;
      if(typeof toast === 'function') toast('FROM: ' + (currentId || 'NONE'), '');

      // 3. If on Title — let app exit naturally (do NOT re-push)
      if(currentId === 's-title'){
        isHandlingPopstate = false;
        return;
      }

      // 4. Stop active quiz timer if any
      if(currentId === 's-play' && typeof G !== 'undefined' && G.gameActive){
        G.gameActive = false;
        if(G.interval) clearInterval(G.interval);
      }

      // 5. Stop active pub timer if any
      if(currentId === 's-pub' && typeof pubSession !== 'undefined' && pubSession){
        if(pubSession.turnTimer) clearInterval(pubSession.turnTimer);
        pubSession = null;
      }

      // 6. Default: navigate to Title
      go('s-title');

      // 7. Re-push so subsequent BACK fires popstate again (not exit)
      try { history.pushState({screen: 'app'}, '', ''); } catch(e){}
    } catch(err){
      console.error('popstate handler error:', err);
      if(typeof toast === 'function') toast('ERR: ' + err.message, 'err');
    } finally {
      isHandlingPopstate = false;
    }
  });
