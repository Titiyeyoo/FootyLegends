...
const NARRATIVES = {
  ...
  foxes:{name:"FOXES 2016",emoji:"🦊",cat:"club",iconic:5,players:["Jamie Vardy","Danny Drinkwater","Marc Albrighton","Danny Simpson","Ben Chilwell","Nathan Dyer"]},
  ...
  eagles:{name:"EAGLES RISING",emoji:"🦅",cat:"club",iconic:4,players:["Dean Henderson","Marc Guehi","Tyrick Mitchell","Joel Ward","Adam Wharton","Eberechi Eze","Nathaniel Clyne","Will Hughes","Eddie Nketiah","Ben Chilwell"]},
  citydynasty:{name:"CITY DYNASTY",emoji:"💙",cat:"club",iconic:3,players:["Phil Foden","John Stones","Kyle Walker","Jack Grealish","Kalvin Phillips","Raheem Sterling","Fabian Delph","James Milner","Joe Hart","Rico Lewis","James Trafford","Nico O'Reilly"]},
...
/* ── ALSO NOTICED ─ connector łączy podtytuł z głównym tytułem (nie przypadkowa pieczątka) ── */
const ALSO_CLUB_PHRASES = [
  c => "also built around a "+c+" core",
  c => "with a "+c+" spine in support",
  c => "and a strong "+c+" presence behind it",
  c => "plus a "+c+" backbone",
];
const ALSO_ERA_PHRASES = {
  C: ["with the rest from the old days","also leaning on throwback names","and a vintage backbone behind them"],
  G: ["with the rest from the golden era","also leaning on golden-era names","and a peak-years backbone behind them"],
  M: ["with the rest from the modern game","also leaning on today\'s lot","and a modern backbone behind them"],
};
...
/* ── REVEAL ───────────────────────────────────────────────────────── */
/* ── ALSO NOTICED: druga spójność w RESZCIE składu (poza rdzeniem). Komentarz, nie nagroda.
   Bramka (w lockXI): rdzeń główny = 4–5. Tu reszta: KLUB(c[0]) ≥4  LUB  ERA ≥6 (cala reszta jednej epoki).
   Tie-break klub > era. Jedna obserwacja. Zero punktów. */
function detectAlso(xiPlayers, coreSet, excludeClub){
  ...
  if(topClubN >= 4){
    ...
    const phrase = ALSO_CLUB_PHRASES[rnd(ALSO_CLUB_PHRASES.length)](clubDisplay(topClub));
    return {kind:"club", key:topClub, count:topClubN, members, sub: ARCH_CLUB_EMOJI+" "+phrase};
  }
  if(topEraN >= 6){   // era: cala reszta z jednej epoki (pomiar: >=4 zalewa 41%, >=6 ~11%)
    const members = new Set(rest.filter(p => p.e===topEra).map(p=>p.n));
    const arr = ALSO_ERA_PHRASES[topEra] || ALSO_ERA_PHRASES.G;
    const phrase = arr[rnd(arr.length)];
    return {kind:"era", key:topEra, count:topEraN, members, sub: (ARCH_ERA_EMOJI[topEra]||"⭐")+" "+phrase};
  }
  return null;
}
...
function lockXI(){
  ...
  // podtytuł: ALSO NOTICED (połączony connectorem z tytułem) > proza interpretRest > eraDescriptor
  if(also){
    html += '<div class="xad-flavor xad-also">'+also.sub+'</div>';
  }else if(det.main){
    const rest = interpretRest(xiPlayers, det.main);
