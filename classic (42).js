...
(function(){
  ...
  // pasek akcji POD lista graczy — tylko REMOVE (BACK zbedny: klik poza panel zamyka)
  function clRenderActions(){
    const bar=$("cl-actions"); if(!bar) return;
    if(CL.target==null){ bar.innerHTML=""; bar.style.display="none"; return; }
    const slot=CL.slots[CL.target];
    if(!slot.player){ bar.innerHTML=""; bar.style.display="none"; return; }
    bar.innerHTML='<button class="cl-act cl-act-rm" onclick="clClear()">REMOVE</button>';
