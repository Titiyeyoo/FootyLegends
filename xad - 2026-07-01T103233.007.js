...
function flBuildDnaCard(cb){
  ...
  ctx.fillStyle="#e9c46a"; ctx.font="900 62px Arial, sans-serif";
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="2px";
  ctx.fillText("MY FOOTY LEGEND", W/2, 132);
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="1px";
  ctx.fillStyle="#8a8576"; ctx.font="bold 26px Arial, sans-serif";
  ctx.fillText("OFFICIAL FOOTY DNA  \u00b7  "+d.games+" MATCHES", W/2, 178);
  ...
  // ── radar (wysrodkowany) ──
  var cx=W/2, cy=468, R=206, OFF=46, ang=[-90,-18,54,126,198];
  ...
  hr(760);
  // ── type hero (OSTRO — bez glow) ──
  ctx.fillStyle="#7d7a68"; ctx.font="bold 24px Arial, sans-serif";
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="6px";
  ctx.fillText("YOU ARE", W/2, 826);
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="1px";
  ctx.fillStyle="#ffd54a"; var ns=90; ctx.font="900 "+ns+"px Arial, sans-serif";
  while(ctx.measureText(ty.name).width>W-160 && ns>50){ ns--; ctx.font="900 "+ns+"px Arial, sans-serif"; }
  ctx.fillText(ty.name, W/2, 918);
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
  ctx.fillStyle="#8a8576"; ctx.font="500 30px Arial, sans-serif";
  FL.wrap(ctx, ty.sub, W-220, 2).forEach(function(ln,k){ ctx.fillText(ln, W/2, 972+k*38); });
  hr(1040);
  ...
  for(var r=0;r<rows.length;r++){ var yy=sy+r*lineH;
    ctx.textAlign="right"; ctx.fillStyle="#7d7a68"; ctx.fillText(rows[r][0], labelX, yy);
