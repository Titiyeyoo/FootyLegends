...
function drawSquad(){
  ...
  if(!dice){ POOL=buildPool(); REROLL_USED=false; DEALT=true; POS_FILTER=null; TARGET_SLOT=null; render(); flash(""); const sx=el("s-xad"); const pin=()=>{ if(sx) sx.scrollTop=0; try{ window.scrollTo(0,0); }catch(e){} }; pin(); requestAnimationFrame(pin); setTimeout(pin,380); return; }
  ROLLING = true;
  ...
  setTimeout(()=>{
    ...
    const sx = el("s-xad");
    const pinTop = ()=>{ if(sx) sx.scrollTop = 0; try{ window.scrollTo(0,0); }catch(e){} };
