...
function showXadPopup(storyKey, tac){
  ...
  if(info){
    ...
    inner += '<div class="xsp-top">NEW ENTRY ADDED<br>TO YOUR LEGACY</div>'+
             ...
             (rl ? '<div class="xsp-rar">'+rl+'</div>' : '');
  }
  if(tac){
    if(!info) inner += '<div class="xsp-top">TACTICAL DISCIPLINE</div>';
    inner += '<div class="xsp-tac">'+tac.icon+' '+tac.label+' · '+tac.m+'/10</div>';
  }
...
function lockXI(){
  ...
  const tac = tacMatches === 10 ? {label:"TACTICAL PURIST", icon:"🧠", m:tacMatches}
            : tacMatches >= 8  ? {label:"WELL ORGANISED", icon:"📋", m:tacMatches}
            : null;
