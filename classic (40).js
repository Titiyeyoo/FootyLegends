...
(function(){
  ...
  // pasek akcji POD lista graczy (remove / BACK) — duze, wygodne pod kciuk
  function clRenderActions(){
    const bar=$("cl-actions"); if(!bar) return;
    if(CL.target==null){ bar.innerHTML=""; bar.style.display="none"; return; }
    const slot=CL.slots[CL.target];
    let h="";
    if(slot.player){
      h+='<button class="cl-act cl-act-rm" onclick="clClear()">REMOVE</button>';
    }
    h+='<button class="cl-act cl-act-back" onclick="window.clCancelPick&&clCancelPick()">BACK</button>';
    bar.innerHTML=h;
  ...
  window.clNewXI = function(kind, key){
    ...
    if(cur > 0 && !confirm("Start a new XI?\n\nCurrent XI will be discarded.")) return;
    clBuildSlots();
  ...
  window.clCloseNew = function(){ var b=$("cl-new-menu"); if(b) b.style.display="none"; };
  ...
  function clRenderList(){
    ...
    // TOP PICKS: bez wpisanego zapytania kozacy na dana pozycje ida na gore listy.
    // Gracz nie musi nic pisac — od razu widzi Shearera, a nie Aarona Lennona.
    var slotPos = (CL.target!=null && CL.slots[CL.target]) ? CL.slots[CL.target].pos : null;
    var topR = (!q && slotPos) ? clTopRank(slotPos) : null;
