...
function xadShareXI(){
  ...
  ctx.fillText(String(S.score), W/2, 200);
  // tytuł (emoji + nazwa)
  ctx.fillStyle="#ffd600"; ctx.font="bold 46px Arial, sans-serif";
  ctx.fillText(((S.emoji?S.emoji+" ":"")+S.title), W/2, 272);
  // boisko
  const pW=520, pH=Math.round(pW*4/3), pX=(W-pW)/2, pY=305;
  ...
  (S.slots||[]).forEach(function(s){
    ...
    // nazwisko (zmniejsz jeśli za długie)
    let fs=22; ctx.font="bold "+fs+"px Arial, sans-serif";
    while(ctx.measureText(s.name).width>150 && fs>13){ fs--; ctx.font="bold "+fs+"px Arial, sans-serif"; }
    ctx.fillStyle="#ffffff"; ctx.fillText(s.name, cx, cy+r+24);
  });
  // stopka
  ctx.fillStyle="#ffd600"; ctx.font="bold 30px Arial, sans-serif";
  ctx.fillText("XI AFTER DARK", W/2, pY+pH+62);
  ctx.fillStyle="#9a9a9a"; ctx.font="bold 36px Arial, sans-serif";
  ctx.fillText("Can you beat "+S.score+"?", W/2, pY+pH+114);
