...
(function(){
  ...
  function clBuildCard(cb){
    ...
    FL.drawHero(ctx,{ emoji:(story&&story.emoji)||"", name:(story&&story.name)||"", tagline:(story&&story.desc)||"", rest:(story&&story.rest)||"" });
    FL.drawScore(ctx,{ kicker: clCardTitle(), score:null });
    FL.drawPitch(ctx);
    const sized = ENGLAND_SHIRT_SVG.replace("<svg ", '<svg width="108" height="108" ');
    const img=new Image();
    function drawPlayers(haveImg){
      const shW=104, shH=104;
      xi.forEach(function(s){
        ...
        ctx.beginPath(); ctx.ellipse(cx, cy+46, 24, 7, 0, 0, Math.PI*2); ctx.fillStyle="rgba(0,0,0,0.6)"; ctx.fill(); ctx.restore();
        ...
        const eh=ERA_HEX[s.player.e]; if(eh){ ctx.beginPath(); ctx.arc(cx+30,cy-38,11,0,Math.PI*2); ctx.fillStyle=eh; ctx.fill(); ctx.lineWidth=2; ctx.strokeStyle="#000"; ctx.stroke(); }
        // #4 długie nazwiska: wyższy minimalny font (równy rząd), pełne nazwisko
        let nm=clLast(s.player.n); let fs=26; ctx.font="bold "+fs+"px Arial, sans-serif";
        while(ctx.measureText(nm).width>132 && fs>18){ fs--; ctx.font="bold "+fs+"px Arial, sans-serif"; }
        ctx.textAlign="center"; ctx.lineJoin="round";
        // #2 ciemny kontur pod nazwiskiem
        ctx.lineWidth=4; ctx.strokeStyle="rgba(0,0,0,0.85)"; ctx.strokeText(nm, cx, cy+shH/2+10);
        ctx.fillStyle="#ffffff"; ctx.fillText(nm, cx, cy+shH/2+10);
      });
