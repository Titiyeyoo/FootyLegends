...
window.FLCard = (function(){
  var W=1080, H=1350, PITCH={x:130,y:452,w:820,h:656};
  ...
  function frame(ctx,opts){
    ...
    ctx.fillStyle="#8a9a8c"; ctx.font="bold 26px Arial, sans-serif"; ctx.fillText(url, W/2, H-72);
    if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
  }
  function drawPitch(ctx){
    var pX=PITCH.x,pY=PITCH.y,pW=PITCH.w,pH=PITCH.h, cx=pX+pW/2;
    ctx.save(); ctx.shadowColor="rgba(0,0,0,0.55)"; ctx.shadowBlur=44; ctx.shadowOffsetY=16;
    var grass=ctx.createLinearGradient(0,pY,0,pY+pH); grass.addColorStop(0,"#2c722a"); grass.addColorStop(1,"#06200d");
    ctx.fillStyle=grass; roundRect(ctx,pX,pY,pW,pH,16); ctx.fill(); ctx.restore();
    ctx.save(); roundRect(ctx,pX,pY,pW,pH,16); ctx.clip();
    for(var i=0;i<10;i++){ ctx.fillStyle = i%2 ? "rgba(255,255,255,0.022)":"rgba(0,0,0,0.05)"; ctx.fillRect(pX,pY+i*(pH/10),pW,pH/10); }
    function pool(fx,fy,fr,c0,c1){ var px=pX+fx*pW, py=pY+fy*pH, r=fr*pW; var g=ctx.createRadialGradient(px,py,0,px,py,r); g.addColorStop(0,c0); g.addColorStop(1,c1); ctx.fillStyle=g; ctx.fillRect(pX,pY,pW,pH); }
    pool(0.5,0.86,0.47,"rgba(67,152,74,0.20)","rgba(67,152,74,0)");
    pool(0.07,0.10,0.42,"rgba(182,242,212,0.26)","rgba(182,242,212,0)");
    pool(0.93,0.10,0.42,"rgba(182,242,212,0.26)","rgba(182,242,212,0)");
    pool(0.5,0.34,0.52,"rgba(116,222,110,0.44)","rgba(116,222,110,0)");
    ctx.fillStyle="rgba(223,244,230,0.12)";
    ctx.beginPath(); ctx.ellipse(pX+0.07*pW,pY+0.02*pH,0.26*pW,0.13*pH,0,0,Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(pX+0.93*pW,pY+0.02*pH,0.26*pW,0.13*pH,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle="rgba(210,240,210,0.10)";
    ctx.beginPath(); ctx.ellipse(cx,pY+0.32*pH,0.52*pW,0.11*pH,0,0,Math.PI*2); ctx.fill();
    var pvg=ctx.createRadialGradient(cx,pY+0.42*pH,0.30*pW,cx,pY+0.42*pH,0.78*pW); pvg.addColorStop(0,"rgba(3,18,6,0)"); pvg.addColorStop(1,"rgba(3,18,6,0.66)");
    ctx.fillStyle=pvg; ctx.fillRect(pX,pY,pW,pH);
    ctx.strokeStyle="rgba(255,255,255,0.30)"; ctx.lineWidth=3;
    roundRect(ctx,pX+10,pY+10,pW-20,pH-20,8); ctx.stroke();
    ctx.beginPath(); ctx.arc(cx,pY+10,0.13*pW,0,Math.PI); ctx.stroke();
    var bxW=0.5*pW,bxH=0.16*pH; ctx.strokeRect(cx-bxW/2,pY+pH-bxH-10,bxW,bxH);
    var syW=0.24*pW,syH=0.07*pH; ctx.strokeRect(cx-syW/2,pY+pH-syH-10,syW,syH);
    ctx.restore();
  }
  function drawHero(ctx,o){
    ...
    if(name){
      ...
      ctx.fillStyle="#e8c860"; ctx.fillText(full, W/2, 152);
    }
    var y=210;
  ...
  function drawScore(ctx,o){
    o=o||{}; ctx.textAlign="center";
    var ky=PITCH.y-118;
    ctx.strokeStyle="rgba(201,167,58,0.45)"; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(W/2-130,ky); ctx.lineTo(W/2+130,ky); ctx.stroke();
    if(o.score!=null && o.score!==""){
      if(ctx.letterSpacing!==undefined) ctx.letterSpacing="6px";
      ctx.fillStyle="#c9a73a"; ctx.font="bold 26px Arial, sans-serif"; ctx.fillText(o.kicker||"", W/2, PITCH.y-76);
      if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
      var sc=String(o.score);
      ctx.font="900 74px Arial, sans-serif"; var w1=ctx.measureText(sc).width;
      ctx.font="bold 37px Arial, sans-serif"; var w2=ctx.measureText(" PTS").width;
      var startX=W/2-(w1+w2)/2;
      ctx.textAlign="left";
      ctx.fillStyle="#ffffff"; ctx.font="900 74px Arial, sans-serif"; ctx.fillText(sc, startX, PITCH.y-22);
      ctx.fillStyle="#c9a73a"; ctx.font="bold 37px Arial, sans-serif"; ctx.fillText(" PTS", startX+w1, PITCH.y-22);
      ctx.textAlign="center";
    } else {
      if(ctx.letterSpacing!==undefined) ctx.letterSpacing="2px";
      ctx.fillStyle="#e8c860"; ctx.font="900 40px Arial, sans-serif"; ctx.fillText(o.kicker||"", W/2, PITCH.y-42);
      if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
    }
  }
