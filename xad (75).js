...
function lockXI(){
  ...
  // ── Ujawnienie historii: piłka skacze po rdzeniu i "zapala" graczy (po werdykcie) ──
...
async function xadStoryReveal(auto){
  ...
  if(rm){ pts.forEach(function(pt){ pt.dot.classList.remove("story-dormant"); pt.dot.classList.add("story-lit"); }); return; }
  pts.forEach(function(pt){ pt.dot.classList.add("story-dormant"); pt.dot.classList.remove("story-lit"); });
  ...
  if(!ball){ ball=document.createElement("div"); ball.className="xad-story-ball"; pitch.appendChild(ball); }
  ...
  ball.style.opacity="0";        // piłka znika, rdzeń zostaje zapalony
  await xadWait(320);
  if(ball) ball.remove();
}
