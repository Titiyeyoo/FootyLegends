...
function flFanType(pr){
  if(pr.games<8) return "forming";
  var g=pr.games;
  var ax=flAxisVals(pr.pickSum,pr.availSum,g,11,30);
  // loyalty z koncentracji klubu (jak flRadarData/flProfile) — wczesniej NIE byl kandydatem
  var loyalty = (pr.conc && g) ? Math.round(100*((pr.conc.topClubXI/g)/11 - (pr.conc.topClubPool/g)/30)) : 0;
  // 7 osi typu. Historia = C+G (Golden zwiniety). raw = pkt% (pick-avail).
  var raw = { history:ax.C+ax.G, modern:ax.M, stars:ax.stars, scout:ax.deep, attack:ax.attack, backbone:ax.defense, loyalty:loyalty };
  // baseline+max per os (kalibracja na OBECNYM buildPool — PRZELICZYC przy zmianie generatora)
  var NORM = { history:[0,30], modern:[0,42], stars:[0,13], scout:[0,47], attack:[-2,12], backbone:[2,8], loyalty:[5,32] };
  // normalizacja (jak daleko ku swojemu maksimum) + argmax; szczyt < 0.45 => balanced
  var best="balanced", bv=0.45;
  for(var k in NORM){ var b=NORM[k][0], m=NORM[k][1]; var n=(raw[k]-b)/(m-b); if(n>bv){ bv=n; best=k; } }
  return best;
}
...
window.flRadarData = function(){
  ...
  var life={
    vintage: flPA(pr.pickSum.C, pr.availSum.C, g),
    modern:  flPA(pr.pickSum.M, pr.availSum.M, g),
    stars:   flPA(pr.pickSum.stars, pr.availSum.stars, g),
    deep:    flPA(pr.pickSum.deep, pr.availSum.deep, g),
    loyalty: pr.conc ? flPA(pr.conc.topClubXI, pr.conc.topClubPool, g) : 0
  };
  var rec=pr.recent.slice(-25), rg=rec.length;
  var sC=0,sM=0,sSt=0,sDp=0,sLt=0, aC=0,aM=0,aSt=0,aDp=0,aLa=0;
  rec.forEach(function(r){
    sC+=r.pk[0]; sM+=r.pk[2]; sSt+=r.pk[3]; sDp+=r.pk[4];
    aC+=r.av[0]; aM+=r.av[2]; aSt+=r.av[3]; aDp+=r.av[4];
    sLt+=(r.lt||0); aLa+=(r.la||0);
  });
  var form = rg ? {
    vintage: flPA(sC,aC,rg), modern: flPA(sM,aM,rg),
    stars: flPA(sSt,aSt,rg), deep: flPA(sDp,aDp,rg), loyalty: flPA(sLt,aLa,rg)
  } : life;
  var favEra="—", best=-1;
  ...
  return {games:g, formGames:rg, tier:tier, life:life, form:form, type:flFanType(pr),
...
function buildRadarSVG(life, form){
  ...
  [0.25,0.5,0.75,1].forEach(function(f){
    ...
    svg+='<polygon points="'+p.join(" ")+'" fill="none" stroke="#2c2c28" stroke-width="1"/>';
  });
  for(var i=0;i<5;i++){ var a=ang[i]*Math.PI/180; svg+='<line x1="'+cx+'" y1="'+cy+'" x2="'+(cx+R*Math.cos(a)).toFixed(1)+'" y2="'+(cy+R*Math.sin(a)).toFixed(1)+'" stroke="#2c2c28" stroke-width="1"/>'; }
  svg+='<polygon points="'+flPoly(disp(life),cx,cy,R)+'" fill="rgba(233,196,106,0.22)" stroke="#e9c46a" stroke-width="2"/>';
  ...
  for(var j=0;j<5;j++){ var a2=ang[j]*Math.PI/180, lx=cx+(R+18)*Math.cos(a2), ly=cy+(R+18)*Math.sin(a2);
    ...
    var val=Math.round(Math.max(2,Math.min(100,dl[j])));
    svg+='<text x="'+lx.toFixed(1)+'" y="'+(ly+1).toFixed(1)+'" fill="#9a967f" font-size="9" text-anchor="'+anc+'">'+labels[j]+'</text>';
    svg+='<text x="'+lx.toFixed(1)+'" y="'+(ly+12).toFixed(1)+'" fill="#e9c46a" font-size="9" text-anchor="'+anc+'">'+val+'</text>'; }
  return svg+'</svg>';
}
...
window.openLegend = function(){
  ...
  var radar=d.games>0?buildRadarSVG(d.life,d.form):'<div style="padding:46px 0;color:#7d7a68;font-size:12px">Play a match to start your radar.</div>';
  ...
  var facts=flRow("ERA", d.games<5?"\u2014":d.favEra)
          ...
          + flRow("RAREST", rar);
  var ty = (d.games>=10) ? (FL_TYPE[d.type]||FL_TYPE.balanced) : null;
  var typeBlock = ty
    ? '<div style="margin:8px 0 4px">'
      + '<div style="color:#7d7a68;font-size:8px;letter-spacing:3px">YOU ARE</div>'
      + '<div style="color:#ffd54a;font-size:20px;letter-spacing:1px;line-height:1.1;margin-top:6px;text-shadow:0 0 8px rgba(255,213,74,.25)">'+ty.name+'</div>'
      + '<div style="color:#8a8576;font-size:9px;letter-spacing:.5px;margin:8px 0 2px;line-height:1.5">'+ty.sub+'</div>'
      + '</div>'
    : '<div style="margin:14px 0 4px;color:#7d7a68;font-size:10px;letter-spacing:1px;line-height:1.8">IDENTITY FORMING<br><span style="color:#5f5c4e;font-size:9px">type revealed in '+(10-d.games)+' matches</span></div>';
  var shareBlock = ty
    ? '<button onclick="flShareDnaCard()" style="font-family:inherit;margin-top:16px;border:1px solid #e9c46a;color:#e9c46a;background:transparent;border-radius:6px;font-size:11px;letter-spacing:1px;padding:11px 24px;cursor:pointer">SHARE MY DNA</button>'
    : '<div style="margin-top:14px;font-size:9px;color:#5f5c4e;letter-spacing:1px;border:1px dashed #33312a;border-radius:6px;padding:10px 16px;display:inline-block">SHARE unlocks at 10 matches</div>';
  b.innerHTML=
    '<button class="btn-back" onclick="go(\'s-title\')">MENU</button>'+
    '<div class="legend-title">MY FOOTY LEGEND</div>'+
    '<div class="legend-tier">'+tierTxt+' &middot; '+d.games+' matches</div>'+
    '<div class="legend-radar">'+radar+'</div>'+
    '<div class="legend-leg"><b style="color:#e9c46a">DNA</b> &middot; <b style="color:#e2554a">Form</b> ('+d.formGames+')</div>'+
    typeBlock+
...
function flBuildDnaCard(cb){
  ...
  FL.frame(ctx,{url:"footylegendsquiz.co.uk", tier:"legendary"});
  ctx.textAlign="center"; ctx.textBaseline="alphabetic";
  ctx.fillStyle="#e9c46a"; ctx.font="900 64px Arial, sans-serif";
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="2px";
  ctx.fillText("MY FOOTY LEGEND", W/2, 140);
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="1px";
  ctx.fillStyle="#8a8576"; ctx.font="bold 27px Arial, sans-serif";
  ctx.fillText("OFFICIAL FOOTY DNA  \u00b7  "+d.games+" MATCHES", W/2, 184);
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
  // ── radar ──
  var cx=W/2, cy=500, R=222, OFF=44, ang=[-90,-18,54,126,198];
  ...
  ctx.strokeStyle="#2c2c28"; ctx.lineWidth=1.5;
  ...
  for(var j=0;j<5;j++){ var a2=ang[j]*Math.PI/180, lx=cx+(R+OFF)*Math.cos(a2), ly=cy+(R+OFF)*Math.sin(a2);
    ...
    var val=Math.round(Math.max(2,Math.min(100,dl[j])));
    ctx.fillStyle="#9a967f"; ctx.fillText(labels[j], lx, ly-13);
    ctx.fillStyle="#e9c46a"; ctx.fillText(""+val, lx, ly+14); }
  ctx.textBaseline="alphabetic"; ctx.textAlign="center";
  // ── type hero ──
  var t0=800;
  ctx.fillStyle="#7d7a68"; ctx.font="bold 26px Arial, sans-serif";
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="6px";
  ctx.fillText("YOU ARE", W/2, t0);
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="1px";
  ctx.fillStyle="#ffd54a"; var ns=88; ctx.font="900 "+ns+"px Arial, sans-serif";
  while(ctx.measureText(ty.name).width>W-180 && ns>48){ ns--; ctx.font="900 "+ns+"px Arial, sans-serif"; }
  ctx.save(); ctx.shadowColor="rgba(255,213,74,0.4)"; ctx.shadowBlur=26; ctx.fillText(ty.name, W/2, t0+84); ctx.restore();
  if(ctx.letterSpacing!==undefined) ctx.letterSpacing="0px";
  ctx.fillStyle="#8a8576"; ctx.font="500 30px Arial, sans-serif";
  FL.wrap(ctx, ty.sub, W-220, 2).forEach(function(ln,k){ ctx.fillText(ln, W/2, t0+132+k*38); });
  // ── stats (dwie kolumny) ──
  ...
  var sy=1058, lineH=56, labelX=W/2-296, valueX=W/2-30;
  ctx.font="bold 32px Arial, sans-serif";
  for(var r=0;r<rows.length;r++){ var yy=sy+r*lineH;
    ctx.strokeStyle="rgba(255,255,255,0.06)"; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(W/2-296,yy+16); ctx.lineTo(W/2+296,yy+16); ctx.stroke();
    ctx.textAlign="left"; ctx.fillStyle="#7d7a68"; ctx.fillText(rows[r][0], labelX, yy);
    ctx.fillStyle="#f3efe2"; ctx.fillText(rows[r][1], valueX, yy); }
...
function lockXI(){
  ...
  try{
    ...
    flTrack("game_complete", {mode:"xad", score:score, story:storyKey||"none", tier:_tier});
    if(storyIsNew && storyKey) flTrack("story_discovered", {story:storyKey, tier:_tier});
  }catch(e){}
...
window.FLCard = (function(){
  ...
  function frame(ctx,opts){
    ...
    if(_glow){ ctx.save(); ctx.shadowColor=_glow; ctx.shadowBlur=34; ctx.strokeStyle=_bc; ctx.lineWidth=4; roundRect(ctx,M,M,W-2*M,H-2*M,28); ctx.stroke(); ctx.stroke(); ctx.restore(); }
