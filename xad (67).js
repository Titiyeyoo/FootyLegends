  ...
  function draw(haveImg){
    S.slots.forEach(function(s){
      ...
      if(s.shirt66 && haveImg){
        ...
        // 1) ciemne tło dysku (bez glow)
        ctx.beginPath(); ctx.arc(ccx,ccy,R,0,Math.PI*2); ctx.fillStyle=st.bg; ctx.fill();
        // 2) miękki glow Z OBRYSU (nie z całego dysku → ostro, nie bańka)
      ...
      ctx.fillStyle=(s.shirt66 ? "#ffffff" : st.pname); ctx.textAlign="center"; ctx.fillText(nm, cx, cy+42);
    });
