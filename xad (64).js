...
function lockXI(){
  ...
  try{
    LAST_SHARE = {
      score: score,
      title: det.main ? NARRATIVES[det.main].name : (archetype ? archetype.name : "MIXED XI"),
      emoji: det.main ? (det.main==="class92" ? "🏴" : NARRATIVES[det.main].emoji) : (archetype ? archetype.emoji : ""),
      slots: SLOTS.map(s => s.player ? {x:s.x, y:s.y, pos:s.pos, grp:s.p, name:lastName(s.player.n)} : null)
    };
  }catch(e){}
